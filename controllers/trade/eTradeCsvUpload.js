...
const eTradeKeys = ["Symbol", "Status", "Fill", "Description", "ID", "Time"];

const eTradeCsvUpload = async (req, res, next) => {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      try {
        ...
        for (const file of files?.csv || []) {
          try {
            await new Promise((resolve, reject) => {
              ...
              stream.on(
                "data",
                (data) => data?.Status === "Filled" && results.push(data)
              );
            ...
            // Validate CSV data
            if (
              !results.every((item) => eTradeKeys.every((key) => key in item))
            ) {
              const missingKeys = eTradeKeys.filter(
        ...
        let allOrderIds = [];
        const trades = uniqueExecutions?.map((data) => {
          const date = formatEtradeCsvDate(data?.["Time"], timeZone);
          const assetClass =
            data?.["Description"]?.split(" ")?.includes("Puts") ||
            data?.["Description"]?.split(" ")?.includes("Calls")
              ? "option"
              : "stocks";
          const side = data["Description"]?.split(" ")?.[0].toLowerCase();
          const quantity = Math.abs(
            convertToNumber(data["Fill"]?.split("@")?.[0]?.trim())
          );
          const price = Math.abs(
            convertToNumber(data["Fill"]?.split("@")?.[1])
          );
...
module.exports = eTradeCsvUpload;
